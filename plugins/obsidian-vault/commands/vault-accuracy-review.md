...
Check *every* factual claim in each note, not a subset:
...
- Correct any errors or outdated information in-place. When a correction references another vault note, link it as a `[[wikilink]]` (resolve the exact filename; use `[[Note#Section]]` for a specific section) per CONVENTIONS.md.
- Add or update a `reviewed` frontmatter property set to today's date (`YYYY-MM-DD`). Even a fully-correct note gets its `reviewed` bumped — the verification itself has value.
